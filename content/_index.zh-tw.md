    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            iframe.onload = function() {
                ...
                // 加載完成後，向iframe發送消息請求高度調整
                try {
                    iframe.contentWindow.postMessage({ type: 'request-height' }, 'https://zhenxun.sm.doctor/index-zh-tw.html');
                } catch (e) {
                    console.log('無法向iframe發送高度請求消息:', e);
                }
            };
            // 處理加載超時情況
            setTimeout(() => {
                if (loader && loader.style.display !== 'none') {
                    loader.innerHTML = '<p style="color: #666; font-size: 16px;">載入較慢，請稍候...</p>';
                }
            }, 5000);
        });
    </script>
    ...
    if (iframe && iframe.contentWindow) {
        try {
            ...
            iframe.contentWindow.postMessage({ 
                ...
                theme: currentTheme,
                timestamp: Date.now()
            }, "https://zhenxun.sm.doctor/index-zh-tw.html");
            
            // 记录最后发送的主题
            iframe.lastTheme = currentTheme;
            
            console.log(`已將主題發送到iframe: ${currentTheme}`);
        } catch (e) {
            console.error("無法向iframe發送消息:", e);
            // 添加重試機制
...
function handleIframeMessage(event) {
    // 验证消息来源
    if (event.origin !== "https://zhenxun.sm.doctor/index-zh-tw.html") {
        return;
    }
    
    // 处理iframe的确认消息
    if (event.data && event.data.type === 'theme-ack') {
        console.log(`iframe已確認收到主題: ${event.data.theme}`);
    }
    ...
    if (event.data && event.data.type === 'height-update' && typeof event.data.height === 'number') {
        ...
        try {
            ...
            console.error('通過消息調整iframe高度時出錯:', e);
        }
    }
}
